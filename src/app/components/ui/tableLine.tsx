...
const TableComponent: React.FC = () => {
  ...
  const machines: Machine[] = [
    { line: 1, name: 'Machine 1', temperature: '30 °C', status: 'Safe', statusColor: 'bg-teal-400' },
    { line: 1, name: 'Machine 2', temperature: '64 °C', status: 'Medium', statusColor: 'bg-yellow-400' },
    { line: 2, name: 'Machine 3', temperature: '95 °C', status: 'Unsafe', statusColor: 'bg-red-400' },
    { line: 2, name: 'Machine 4', temperature: '68 °C', status: 'Medium', statusColor: 'bg-yellow-400' },
    { line: 3, name: 'Machine 5', temperature: '', status: '', statusColor: '' },
  ];
  ...
  return (
    <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
      <div className="p-4 bg-[#4D4B6C] text-center text-white rounded-t-lg">
        <p className="text-sm">
          <span className="text-green-400">{'< 60 °C = Safe (Green)'}</span><br />
          <span className="text-yellow-400">{'60 °C & 90 °C = Medium (Yellow)'}</span><br />
      ...
      <div className="overflow-x-auto whitespace-nowrap pl-4 py-2 text-md bg-[#3E3B64] text-white">
        <div className="flex justify-between items-center">
          <div className="flex space-x-4">
            ...
            <span
              className={`cursor-pointer ${activeTab === 'Line' ? 'text-blue-400 border-b-2 border-blue-400' : ''}`}
          ...
          <table className="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
            <thead className="text-xs uppercase bg-[#3E3B64] text-white border-b">
              <tr>
                ...
                <th scope="col" className="px-6 py-3">Machines</th>
                <th scope="col" className="px-6 py-3">Temperature</th>
                <th scope="col" className="px-6 py-3">Temperature Status</th>
              ...
              {filteredMachines.map((machine, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    {machine.status && (
                      <span className={`px-4 py-2 rounded-full ${machine.statusColor} text-blue-900 font-bold`}>
                        {machine.status}
                      </span>
                    )}
                  </td>
                  <td className="px-6 py-4 flex space-x-2">
                    ...
                    <button className="bg-pink-500 hover:bg-pink-700 text-white font-bold py-2 px-4 rounded">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            <tbody>
              {lines.map((line, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    {line.status && (
                      <span className={`px-4 py-2 rounded-full ${line.lineColor} text-blue-900 font-bold`}>
                        {line.status}
                      </span>
                    )}
                  </td>
