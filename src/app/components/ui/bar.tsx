...
const PartTypesComponent = () => {
  ...
  return (
    <div className="bg-[#3E3B64] p-5 rounded-lg text-white w-90 font-sans">
      <h2 className="text-xl mb-2 border-b border-gray-400 pb-1 font-bold">Total Part Types</h2>
      ...
      <div className="relative h-4 rounded-full overflow-hidden bg-gray-700">
        <div className="absolute h-full bg-green-400" style={{ width: `${(unused / totalParts) * 100}%` }}></div>
        <div className="absolute h-full bg-blue-400" style={{ width: `${(ordered / totalParts) * 100}%`, left: `${(unused / totalParts) * 100}%` }}></div>
        <div className="absolute h-full bg-red-400" style={{ width: `${(used / totalParts) * 100}%`, left: `${((unused + ordered) / totalParts) * 100}%` }}></div>
      </div>
